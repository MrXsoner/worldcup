...
echo  "$($PSQL "SELECT SUM(winner_goals) + SUM(opponent_goals) AS suma_sum FROM games")"
...
echo "$($PSQL "SELECT DISTINCT teams.name FROM teams JOIN games ON teams.team_id = games.winner_id ORDER BY name")"
